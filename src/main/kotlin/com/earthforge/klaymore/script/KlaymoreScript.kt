 ...
 * - 解决 Kotlin `object` 单例导致的多实例字段互相覆盖问题：
 *   每次 `spawnChild` 都会 `newInstance()` 出全新的 `KlaymoreScript` 子类实例。
 * - 提供 IDE 代码提示：target / container / net 等字段在基类中声明，
 *   子类直接可见，无需靠 `lateinit var` 顶层声明触发 IDE 报错。
 *
 * 字段注入由引擎通过反射完成（见 ScriptInjectionUtils.injectFields），
 * 注入顺序：target → container → selfContainer → parent → net。
 */
abstract class KlaymoreScript {
   ...
   *
   * 全局脚本不绑定任何实体/方块，引擎会自动分配一个 Dummy 作为 target。
