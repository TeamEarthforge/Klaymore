...
public class ForgeEventHandler {
    ...
    private static final Set<String> REPORTED_ERROR_KEYS = Collections
        .newSetFromMap(new ConcurrentHashMap<String, Boolean>());

    /** busHashCode|Class全限定名 → 已注册过，避免重复注册到同一 bus */
    private static final Set<String> REGISTERED_KEYS = Collections
        .newSetFromMap(new ConcurrentHashMap<String, Boolean>());
    ...
    public static void registerBridgeFor(Class eventClass, EventBus... buses) {
        ...
        for (EventBus bus : buses) {
            ...
            try {
                ...
                com.earthforge.klaymore.Klaymore.LOG
                    .error("[Klaymore] Failed to register typed bridge for " + cls.getName(), t);
            }
        }
    }
    ...
    private static void registerTypedListener(EventBus bus, Class<? extends Event> eventClass) throws Exception {
        ...
        if (factory != null) {
            ...
            com.earthforge.klaymore.Klaymore.LOG
                .debug("[Klaymore] [FAST] Typed bridge registered: " + eventClass.getName());
            return;
        }
        ...
        if (listenersField != null) {
            ...
            if (listenersMap instanceof java.util.concurrent.ConcurrentHashMap) {
                ...
                java.util.concurrent.ConcurrentHashMap<Object, List<Object>> map = (java.util.concurrent.ConcurrentHashMap<Object, List<Object>>) listenersMap;
                ...
                com.earthforge.klaymore.Klaymore.LOG
                    .debug("[Klaymore] [REFLECT-FALLBACK] Typed bridge injected: " + eventClass.getName());
                return;
            }
        }
        ...
        com.earthforge.klaymore.Klaymore.LOG
            .debug("[Klaymore] [FINAL-FALLBACK] SingleEventBridge registered: " + eventClass.getName());
    }
    ...
    public static class SingleEventBridge {
        ...
        public SingleEventBridge(Class<? extends Event> eventClass) {
        ...
        try {
            ...
            if (REPORTED_ERROR_KEYS.add(key)) {
                com.earthforge.klaymore.Klaymore.LOG.error(
                    "[Klaymore] Error dispatching Forge event " + eventName
                        + ": "
                        + t.getMessage()
    ...
    public static class EntityEventBridge {
        ...
        @SubscribeEvent
        public void onEntity(net.minecraftforge.event.entity.EntityEvent e) {
            dispatch(e);
        }
    }
    ...
    public static class PlayerEventBridge {
        ...
        @SubscribeEvent
        public void onPlayer(net.minecraftforge.event.entity.player.PlayerEvent e) {
            dispatch(e);
        }
    }
    ...
    public static class LivingEventBridge {
        ...
        @SubscribeEvent
        public void onLiving(net.minecraftforge.event.entity.living.LivingEvent e) {
            dispatch(e);
        }
    }
    ...
    public static class BlockEventBridge {
        ...
        @SubscribeEvent
        public void onBlock(net.minecraftforge.event.world.BlockEvent e) {
            dispatch(e);
        }
    }
    ...
    public static class FMLPlayerEventBridge {
        ...
        @SubscribeEvent
        public void onFMLPlayer(cpw.mods.fml.common.gameevent.PlayerEvent e) {
            dispatch(e);
        }
    }
    ...
    public static class WorldEventBridge {
        ...
        @SubscribeEvent
        public void onWorld(net.minecraftforge.event.world.WorldEvent e) {
            dispatch(e);
        }
    }
    ...
    public static class TickEventBridge {
        ...
        @SubscribeEvent
        public void onTick(cpw.mods.fml.common.gameevent.TickEvent e) {
            dispatch(e);
        }
    }
