...
public final class BuiltinTargetExtractors {
    ...
    public static void registerAll() {
        ...
        // ================================================================
        EventTargetExtractorRegistry.registerExtractor(
            cpw.mods.fml.common.gameevent.PlayerEvent.class,
            event -> event.player
        );
        ...
        // ================================================================
        try {
            Class<?> harvestEventCls = Class.forName(
                "net.minecraftforge.event.world.BlockEvent$HarvestDropsEvent");
            EventTargetExtractorRegistry.registerExtractor(
                (Class) harvestEventCls,
                event -> {
                    java.util.List<Object> list = new java.util.ArrayList<>();
                    Object harvester = firstNonNull(
                        reflectGet(event, "harvester"),
                        reflectGet(event, "entityPlayer")
                    );
        ...
        // ================================================================
        // 7. ⛔ 跳过事件（完全不派发，入口直接短路返回）
        //    适合纯客户端高频事件 / 脚本层根本不关心的事件
        //    同时也能规避像 RenderTickEvent 那样与 Kotlin 运行时
        //    类加载顺序冲突导致的 NoClassDefFoundError 刷屏
        // ================================================================
        ...
        // 7.3 如果你发现其他事件也有 Intrinsics 报错或纯客户端没用，按上面方式加一行即可
        // markSkippedIfExists(
